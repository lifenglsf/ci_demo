...
class Udb extends CI_Controller {
	...
	public function v3() {
		$this->load->dbforge();
		$hasField = $this->db->field_exists('province', 'content');
		var_dump($hasField);
		if (!$hasField) {
			$fields = array(
				'province' => array(
		...
		$hasField = $this->db->field_exists('city', 'order');
		if (!$hasField) {
			$fields = array(
				'city' => array(
					'type' => 'INT',
		...
		$hasField = $this->db->field_exists('county', 'order');
		if (!$hasField) {
			$fields = array(
				'county' => array(
					'type' => 'INT',
	...
	public function v4() {
		$this->load->dbforge();
		$hasField = $this->db->field_exists('uid', 'content');
		var_dump($hasField);
		if (!$hasField) {
			$fields = array(
				'uid' => array(
		...
		$hasField = $this->db->field_exists('message', 'order');
		if (!$hasField) {
			$fields = array(
				'message' => array(
					'type' => 'VARCHAR',
		...
		$hasField = $this->db->field_exists('price', 'order');
		if (!$hasField) {
			$fields = array(
				'price' => array(
					'type' => 'varchar',
